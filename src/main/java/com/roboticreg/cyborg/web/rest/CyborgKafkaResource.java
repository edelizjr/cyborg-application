...
@RestController
@RequestMapping("/api/cyborg-kafka")
public class CyborgKafkaResource {

    private final Logger log = LoggerFactory.getLogger(CyborgKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public CyborgKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
